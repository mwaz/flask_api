...
class CategoriesTestCase(unittest.TestCase):
    ...
    def user_register_details(self, email="test@test.com", password="password"):
        """"Method to register a user"""
        user_details = {
                         'email':email,
                         'password':password
                        }
        return self.client().post('yummy_api/v1/auth/register', data=user_details)

    def user_login_details(self, email="test@test.com", password="password"):
        """method to login a registered user"""
        login_details = {
                          'email':email,
                          'password':password
                        }
        return self.client().post('yummy_api/v1/auth/login', data=login_details)

    def test_create_categories(self):
        """Test if the API can create a recipe categ
        ory using [post]"""
        self.user_register_details()#creates a user
        res = self.user_login_details() #log in a user
    ...
    def test_api_can_get_all_recipe_categories(self):
        ...
        self.user_register_details()
        res = self.user_login_details()
        access_token = json.loads(res.data.decode())['access_token']
        get_categories = self.client().post('/yummy_api/v1/categories/', headers=dict(Authorization="Bearer " + access_token), data = self.categories)
        self.assertEqual(get_categories.status_code, 201)
        get_categories = self.client().get('/yummy_api/v1/categories/')
    ...
    def test_api_can_get_category_by_id(self):
        ...
        get_category_by_id = self.client().post('/yummy_api/v1/categories/', headers=dict(Authorization="Bearer " + access_token), data = self.categories)
        ...
        result = self.client().get(
            '/yummy_api/v1/categories/{}'.format(get_result_in_json['id']), headers=dict(Authorization="Bearer " + access_token))
    ...
    def test_api_can_edit_a_recipe_category(self):
        ...
        create_category = self.client().post('/yummy_api/v1/categories/', headers=dict(Authorization="Bearer " + access_token), data={'category_name': 'new_category'})
        self.assertEqual(create_category.status_code, 201)

        edit_category = self.client().put('/yummy_api/v1/categories/1', headers=dict(Authorization="Bearer " + access_token), data={"category_name": "newly_edited_category"})
        self.assertEqual(edit_category.status_code, 200)

        #test to check whether the edited category exists
        results = self.client().get('/yummy_api/v1/categories/1', headers=dict(Authorization="Bearer " + access_token),)
    ...
    def test_categories_deletion(self):
        ...
        create_category = self.client().post('/yummy_api/v1/categories/', headers=dict(Authorization="Bearer " +access_token), data={'category_name': 'new_category_name'})
        self.assertEqual(create_category.status_code, 201)

        delete_result = self.client().delete('/yummy_api/v1/categories/1', headers=dict(Authorization="Bearer " +access_token),)
        ...
        result_if_category_exists = self.client().get('/yummy_api/v1/categories/1', headers=dict(Authorization="Bearer " +access_token),)
